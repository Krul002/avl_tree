	...
	*/
	...
	template <class Key, class T, class Node_Alloc = std::allocator<avl_tree_node<Key, T> > >
		...
		public:
			...
			node*	find_node(node** root, first_type key) {
				node*	tmp = *root;
				while (tmp) {
					if (tmp->first == key)
						return tmp;
			...
			node*	erase_left(node* n) {
				...
				if (replace) {
					std::cout << n->first << " " << replace->first << std::endl;
					if (replace == n->left) {
						std::cout << "\treplace == n->left" << std::endl;
						n->left = replace->left;
						if (replace->left) {
							std::cout << "\tr->l->par = " << replace->left->parent->first << std::endl;
						...
						if (left) {
							std::cout << "\tleft != NULL:" << left->first << std::endl;
							left->left = n->left;
							if (left->left)
						...
						if (replace->parent) {
							std::cout << "\tr->par->r = " << replace->parent->right->first << std::endl;
